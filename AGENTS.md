...
## 🧠 지식 동기화 및 실시간 인수인계 (LLM Wiki OS) — 2026-06-01+

이 프로젝트는 글로벌 헌법(GEMINI.md Tier 2.6)에 따라 `GSF-OS`를 중앙 지식 저장소로 사용합니다. 에이전트는 작업 전후로 다음의 라우팅 규칙과 Eval(검증) 절차를 강제로 수행해야 합니다.

### 1. 디렉토리 라우팅 규칙 (Knowledge Routing)
- **위키 저장소 경로**: `/Users/gsf/.gemini/antigravity/scratch/projects/GSF-OS/` (글로벌 매핑: `projects/GSF-OS/`)
- GSF-Blog와 관련된 모든 아키텍처 결정, 에러 해결 노트(Notion 파이프라인, Astro 빌드 충돌 등), 방어 로직 가이드는 이 프로젝트 내부가 아닌 위 경로의 `Wiki/` 폴더에 단일 저장(SSOT)됩니다.

### 2. Pre-Coding Eval (검색 강제 조항)
- 기능 구현이나 버그 수정을 시작하기 전, 에이전트는 지시받은 태스크와 관련된 핵심 키워드를 추출하여 `grep_search` 툴로 `GSF-OS/Wiki/` 영역을 선행 검색해야 합니다.
- **Eval 통과 조건**: 검색 결과를 바탕으로 기존의 에러(Risk)나 아키텍처 제약사항을 파악했음을 인지한 상태에서만 코딩 단계로 진입할 수 있습니다. (검색 내역이 없더라도 탐색 절차는 필수)

### 3. Post-Coding Save (지식 갱신 의무)
- 유의미한 시행착오 극복이나 구조적 결정을 내렸다면, 에이전트는 이 맥락이 다음 세션에서 휘발되지 않도록 사용자에게 위키 갱신을 제안하거나 스스로 `save` 규칙에 따라 GSF-OS 위키를 업데이트해야 합니다.
- 단, 갱신 시에는 반드시 **위키 승격의 5대 필터(반복 재사용성, 인수인계 필수성, 근거 추적, 리스크 방지, 공통 규칙)** 중 하나 이상을 충족하는지 검토해야 합니다.
